...
if not os.path.exists(simPath): # Should exists, since this is the path to the simulation data
    print(f"Error: The specified simulation path does not exist: {simPath}")
    sys.exit(1)

if not os.path.exists(sampleFolder): # Create the folder if it does not exist
    os.makedirs(sampleFolder)
    print(f"Created folder: {sampleFolder}")
else:
    print(f"Folder already exists: {sampleFolder}")
...
for snap in args.snaps:
    ...
    sample_file = np.vstack((halo_IDs, Mstar.to('Msun').value)).T[SEL, :]

    np.savetxt(sampleFolder + 'sample_' + str(snap) + '.txt', sample_file, fmt = ['%d', '%.6e'], header = header)